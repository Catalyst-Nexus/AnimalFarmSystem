...
export interface DeliveryItem {
  ...
  total_price: number;
  quantity_issuance: number;
  unit_price_issuance: number;
  status: string | null;
...
export const fetchDeliveryItems = async (): Promise<DeliveryItem[]> => {
  ...
  const { data, error } = await module3()
    .from("delivery_items")
    .select("*, category(*), brand(*), unit_delivery:unit!delivery_items_unit_delivery_id_fkey(*), unit_issuance:unit!delivery_items_unit_issuance_id_fkey(*)")
    .order("created_at", { ascending: false });
  if (error) {
